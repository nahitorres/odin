...
from odin.classes import TaskType, DatasetClassification, AnalyzerClassification, Metrics, Curves


class ClassificationBinaryTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        task = TaskType.CLASSIFICATION_BINARY
        ...
        file = open(os.path.join(dir_path, "data/expected_results_BIN.json"), "r")
        ...
        cls.dataset_perfect = DatasetClassification(ds_path, task, props_path, save_graphs_as_png=False,
                                                    properties_file=properties_path)
        file = open(os.path.join(dir_path, "data/perfect_case/expected_results_BIN.json"), "r")
        ...
        cls.dataset_worst = DatasetClassification(ds_path, task, props_path, save_graphs_as_png=False,
                                                  properties_file=properties_path)
        file = open(os.path.join(dir_path, "data/worst_case/expected_results_BIN.json"), "r")
        cls.expected_results_worst = json.load(file)
        file.close()

        cls.category = cls.dataset_generic.get_categories_names()[0]
    ...
    def test_accuracy_evaluation_metric(self):
        self._support_test_accuracy(y_true=[1, 1, 1, 1],
                                    y_pred=[0.6, 0.7, 1, 1],
                                    expected_result=1)

        self._support_test_accuracy(y_true=[1, 1, 1, 0],
                                    y_pred=[0.4, 1, 1, 0.7],
                                    expected_result=0.5)
        ...
        self._support_test_accuracy(y_true=[0, 0, 0, 0],
                                    y_pred=[0, 0, 0, 0],
                                    expected_result=1)
    ...
    def test_error_rate_evaluation_metric(self):
        self._support_test_error_rate(y_true=[1, 1, 1, 1],
                                      y_pred=[0.6, 0.7, 1, 1],
                                      expected_result=0)

        self._support_test_error_rate(y_true=[1, 1, 1, 0],
                                      y_pred=[0.4, 1, 1, 0.7],
                                      expected_result=0.5)
        ...
        self._support_test_error_rate(y_true=[0, 0, 0, 0],
                                      y_pred=[0, 0, 0, 0],
                                      expected_result=0)
    ...
    def test_precision_evaluation_metric(self):
        self._support_test_precision(y_true=[1, 1, 1, 1],
                                     y_pred=[0.6, 0.7, 1, 1],
                                     expected_result=1)

        self._support_test_precision(y_true=[1, 1, 1, 0],
                                     y_pred=[0.4, 1, 1, 0.7],
                                     expected_result=2/3)
        ...
        self._support_test_precision(y_true=[0, 0, 0, 0],
                                     y_pred=[0, 0, 0, 0],
                                     expected_result=0)
    ...
    def test_recall_evaluation_metric(self):
        self._support_test_recall(y_true=[1, 1, 1, 1],
                                  y_pred=[0.6, 0.7, 1, 1],
                                  expected_result=1)

        self._support_test_recall(y_true=[1, 1, 1, 0],
                                  y_pred=[0.4, 1, 1, 0.7],
                                  expected_result=2 / 3)
        ...
        self._support_test_recall(y_true=[0, 0, 0, 0],
                                  y_pred=[0, 0, 0, 0],
                                  expected_result=0)
    ...
    def test_f1_evaluation_metric(self):
        self._support_test_f1(y_true=[1, 1, 1, 1],
                              y_pred=[0.6, 0.7, 1, 1],
                              expected_result=1)

        self._support_test_f1(y_true=[1, 1, 1, 0],
                              y_pred=[0.4, 1, 1, 0.7],
                              expected_result=2 / 3)
        ...
        self._support_test_f1(y_true=[0, 0, 0, 0],
                              y_pred=[0, 0, 0, 0],
                              expected_result=0)
    ...
    def test_set_threshold(self):
        self.analyzer_generic.set_confidence_threshold(0.2)
        self.assertEqual(0.2, self.analyzer_generic._conf_thresh)
    ...
    def _support_test_analyze_property(self, analyzer, expected_results):
        ...
        self.assertEqual(expected_results["analyze_properties"][Metrics.F1_SCORE.value],
                         res[self.category]['all']['value'])
        for m_a_value in ["small", "medium", "large"]:
            self.assertEqual(
                expected_results["analyze_properties"]["meta_annotations"]["size"][m_a_value][
                    Metrics.F1_SCORE.value],
                res[self.category]["size"][m_a_value]["value"])
    ...
    def _support_test_precision_recall_curve(self, analyzer, expected_results):
        res = analyzer.analyze_curve_for_categories(curve=Curves.PRECISION_RECALL_CURVE, show=False)[self.category]
        expected_recall = np.array(expected_results["pr_curve"]["Recall"])
        expected_precision = np.array(expected_results["pr_curve"]["Precision"])
        self.assertTrue((expected_recall == res["x"]).all())
        self.assertTrue((expected_precision == res["y"]).all())
    ...
    def _support_test_f1_curve(self, analyzer, expected_results):
        res = analyzer.analyze_curve_for_categories(curve=Curves.F1_CURVE, show=False)[self.category]
        expected_threshold = np.array(expected_results["f1_curve"]["Threshold"])
        expected_f1 = np.array(expected_results["f1_curve"]["F1"])
        self.assertTrue((expected_threshold == res["x"]).all())
        self.assertTrue((expected_f1 == res["y"]).all())
    ...
    def _support_test_roc_curve(self, analyzer, expected_results):
        res = analyzer.analyze_curve_for_categories(curve=Curves.ROC_CURVE, show=False)[self.category]
        expected_fpr = np.flip(expected_results["roc_curve"]["fpr"])
        expected_tpr = np.flip(expected_results["roc_curve"]["tpr"])
        self.assertTrue((expected_fpr == res["x"]).all())
        self.assertTrue((expected_tpr == res["y"]).all())
    ...
    def _support_test_confusion_matrix(self, analyzer, expected_results):
        cm = analyzer.show_confusion_matrix(categories=[self.category], show=False)
        expected_cm = np.array(expected_results["cm"])
    ...
    def _support_test_base_report(self, analyzer, expected_results):
        ...
        for metric in metrics:
            ...
            self.assertEqual(expected_results["base_report"][metric.value]["total"], report_total.loc["total"][metric.value])
    ...
    def _support_test_calculate_metric_for_category(self, analyzer, expected_results):
        res = analyzer._calculate_metric_for_category(self.category, Metrics.F1_SCORE)

        self.assertEqual(expected_results["analyze_properties"][Metrics.F1_SCORE.value], res["value"])
    ...
    def _support_test_calculate_metric_for_properties_of_category(self, analyzer, expected_results):
        res = analyzer._calculate_metric_for_properties_of_category(self.category, 1, "size", ["small", "medium", "large"], None, Metrics.F1_SCORE)

        for v in ["small", "medium", "large"]:
            self.assertEqual(expected_results["analyze_properties"]["meta_annotations"]["size"][v][Metrics.F1_SCORE.value],
                             res[v]["value"])
    ...
    def _support_test_convert_input_format_for_category(self, dataset, analyzer, expected_results):
        ...
        y_true, y_scores = analyzer._AnalyzerClassification__convert_input_format_for_category(matching, self.category)

        expected_y_true = np.array(expected_results["convert_for_category"]["y_true"])
        expected_y_scores = np.array(expected_results["convert_for_category"]["y_score"])
    ...
    def _support_test_convert_input_confusion_matrix(self, dataset, analyzer, expected_results):
        ...
        expected_y_true = np.array(expected_results["convert_input_cm"]["y_true"])
        expected_y_pred = np.array(expected_results["convert_input_cm"]["y_pred"])

        expected_labels = np.array([1])
        expected_cat_ids = np.array([1])

        matching = analyzer._match_classification_with_ground_truth(gt_ord, proposals)

        y_true, y_pred, labels, cat_ids = analyzer._AnalyzerClassification__convert_input_confusion_matrix(matching, [self.category], None)
    ...
    def _support_test_convert_input_reliability(self, dataset, analyzer, expected_results):
        ...
        expected_y_true = np.array(expected_results["convert_input_reliability"]["y_true"])
        expected_y_pred = np.array(expected_results["convert_input_reliability"]["y_pred"])
        expected_y_score = np.array(expected_results["convert_input_reliability"]["y_score"])
    ...
    def test_tp_distribution_not_supported(self):
        error = self.analyzer_generic.show_true_positive_distribution()
        self.assertEqual(-1, error)

    def test_fp_distribution_not_supported(self):
        error = self.analyzer_generic.show_false_positive_distribution()
        self.assertEqual(-1, error)

    def test_tn_distribution_not_supported(self):
        error = self.analyzer_generic.show_true_negative_distribution()
        self.assertEqual(-1, error)

    def test_fn_distribution_not_supported(self):
        error = self.analyzer_generic.show_false_negative_distribution()
        self.assertEqual(-1, error)
