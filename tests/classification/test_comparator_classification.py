...
from odin.classes import ComparatorClassification, TaskType


class ComparatorClassificationTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.models_props = [('model_A', os.path.join(cls.dir_path, "multilabel/data/predictions")),
                            ('model_B', os.path.join(cls.dir_path, "multilabel/data/predictions"))]

    def setUp(self):
        properties_path = os.path.join(self.dir_path, "properties.json")
        self.comparator = ComparatorClassification(self.ds_path, TaskType.CLASSIFICATION_MULTI_LABEL,
    ...
    def test_ComparatorClassification(self):
        with self.assertRaises(TypeError):
            ComparatorClassification(10, TaskType.CLASSIFICATION_MULTI_LABEL, self.models_props)
        with self.assertRaises(TypeError):
            ComparatorClassification(self.ds_path, 10, self.models_props)
        with self.assertRaises(ValueError):
            ComparatorClassification(self.ds_path, TaskType.OBJECT_DETECTION, self.models_props)
        with self.assertRaises(TypeError):
            ComparatorClassification(self.ds_path, TaskType.CLASSIFICATION_MULTI_LABEL, 10)
        with self.assertRaises(TypeError):
            ComparatorClassification(self.ds_path, TaskType.CLASSIFICATION_MULTI_LABEL, self.models_props,
    ...
    def test_show_true_negative_distribution(self):
        error = self.comparator.show_true_negative_distribution(categories=10)
        self.assertEqual(-1, error)
        error = self.comparator.show_true_negative_distribution(show=10)
        self.assertEqual(-1, error)
    ...
    def test_show_true_negative_distribution_for_categories_for_property(self):
        error = self.comparator.show_true_negative_distribution_for_categories_for_property(10)
        self.assertEqual(-1, error)
        error = self.comparator.show_true_negative_distribution_for_categories_for_property("size", property_values=10)
        self.assertEqual(-1, error)
        error = self.comparator.show_true_negative_distribution_for_categories_for_property("size", categories=10)
        self.assertEqual(-1, error)
        error = self.comparator.show_true_negative_distribution_for_categories_for_property("size", show=10)
        self.assertEqual(-1, error)

    def test_show_false_positive_distribution_for_categories_for_property(self):
        error = self.comparator.show_false_positive_distribution_for_categories_for_property(10)
        self.assertEqual(-1, error)
        error = self.comparator.show_false_positive_distribution_for_categories_for_property("size",
                                                                                            property_values=10)
        self.assertEqual(-1, error)
        error = self.comparator.show_false_positive_distribution_for_categories_for_property("size", categories=10)
        self.assertEqual(-1, error)
        error = self.comparator.show_false_positive_distribution_for_categories_for_property("size", show=10)
        self.assertEqual(-1, error)
